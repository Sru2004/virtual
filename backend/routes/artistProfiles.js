...
router.get('/user/:userId', auth, async (req, res) => {
  try {
    const artistProfile = await ArtistProfile.findOne({ user_id: req.params.userId }).populate('user_id', 'full_name email');
...
router.get('/:id', auth, async (req, res) => {
  try {
    const artistProfile = await ArtistProfile.findById(req.params.id).populate('user_id', 'full_name email');
    if (!artistProfile) {
...
router.put('/:id', auth, [
  ...
  try {
    ...
    const updatedProfile = await ArtistProfile.findByIdAndUpdate(req.params.id, updates, { new: true }).populate('user_id', 'full_name email');
